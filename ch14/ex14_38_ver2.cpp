...
public:
    ...
    bool operator()(const std::string &s) const
        { return s.size() >= _lower && str.size() <= _upper; }
...
int main(void)
{
    // create predicates(謂詞) with various(各種) upper limit.
    std::size_t lower = 1;
    auto uppers = {3u, 4u, 5u, 6u, 7u, 8u, 9u, 10u, 11u, 12u, 13u, 14u};    // u = unsigned int
